program scatter
    ...
    integer :: localsize
    integer :: i, j, row, col, istat, p, charsize
    ...
    call MPI_Init(istat)
    call MPI_Comm_size(MPI_COMM_WORLD, comsize, istat)
    ...
    call MPI_Type_create_subarray(2, sizes, subsizes, starts,        &
                                  ...
                                  newtype, istat)
    call MPI_Type_size(MPI_CHARACTER, charsize, istat)
    extent = localsize*charsize
    begin  = 0
    call MPI_Type_create_resized(newtype, begin, extent, resizedtype, istat)
    call MPI_Type_commit(resizedtype, istat)
    ...
    call MPI_Scatterv(global, counts, displs,   & ! proc i gets counts(i) types from displs(i)
            ...
            root, MPI_COMM_WORLD, istat)           !... from (root,MPI_COMM_WORLD)
    ...
    call MPI_Type_free(newtype,istat)
