module mo_decomp
 ...
 integer :: dims(2), coord(2), comm2d, err
 ...
 subroutine init_decomp
  ...
  call MPI_CART_CREATE(MPI_COMM_WORLD,2,dims,period,reorder,comm2d,ierr)
  call MPI_COMM_RANK(comm2D,my_rank, ierr)
  call MPI_CART_COORDS(comm2d,my_rank,2,coord,ierr)
  ...
  call MPI_Cart_shift(comm2d,0,1,NeighBor(W),NeighBor(E),ierr)

  ! Bottom/South and Upper/North neigbors
  call MPI_Cart_shift(comm2d,1,1,NeighBor(S),NeighBor(N),ierr)
  !if(my_rank==0) then
  !do i=0,nproc-1
  !   print*,'P:',i,' coordinates are',all_coord(2*(i)+1), all_coord(2*(i)+2)
  !enddo
  !endif
!
  MOIE = NX_GLOBAL
...
!      ENDDO
  ...
  do i=0,nproc-1
   if(i==my_rank) then
     ...
     print*,'  ',my_rank,'    MYPOS   ',MYPOS
     print*,'  ',my_rank,'    NEIGHBOR',NEIGHBOR
   else
